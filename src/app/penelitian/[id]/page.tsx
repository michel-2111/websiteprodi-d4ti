...
import { ArrowLeft, Search, Calendar, Users, User } from "lucide-react";
        ...
        <div className="min-h-screen bg-zinc-50 pb-24">
            ...
            <div className="bg-white border-b pt-8 pb-6">
                <div className="container mx-auto px-4 max-w-4xl">
                    ...
                    <div className="flex items-center gap-2 mb-4">
                        <span className="inline-flex items-center gap-1.5 bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide">
                            <Search className="h-3.5 w-3.5" /> Penelitian
                        </span>
            ...
            <div className="container mx-auto px-4 max-w-4xl mt-8">
                ...
                {data.gambarUrls && data.gambarUrls.length > 0 && (
                    <div className="mb-10">
                        <h3 className="text-sm font-semibold text-zinc-500 uppercase tracking-wider mb-4">Dokumentasi Kegiatan ({data.gambarUrls.length})</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div className="md:col-span-2 space-y-6">
                        <div className="bg-white p-6 md:p-8 rounded-2xl border shadow-sm">
                            <h3 className="text-xl font-bold mb-4 text-zinc-900 border-b pb-4">Abstrak / Deskripsi</h3>
                            {data.deskripsi ? (
                                <p className="text-zinc-600 leading-relaxed whitespace-pre-line text-justify">
                                    {data.deskripsi}
                                </p>
                            ) : (
                                <p className="text-zinc-400 italic">Deskripsi tidak dilampirkan untuk penelitian ini.</p>
                            )}
                        </div>
                    </div>

                    {/* Kolom Kanan: Tim Peneliti */}
                    <div className="md:col-span-1 space-y-6">
                        <div className="bg-white p-6 rounded-2xl border shadow-sm sticky top-6">
                            <h3 className="font-bold text-zinc-900 border-b pb-3 mb-4">Tim Peneliti</h3>
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <Link href={`/dosen-publik/${data.ketua.userId}`} className="flex items-center gap-3 p-2 -mx-2 rounded-lg hover:bg-zinc-50 transition-colors group">
                                        ...
                                        <div>
                                            <p className="font-semibold text-zinc-900 text-sm group-hover:text-blue-600 transition-colors">{data.ketua.user.name}</p>
                                        </div>
                                    </Link>
                                </div>
                                        ...
                                        <div className="space-y-1">
                                            {data.anggota.map(anggota => (
                                                <Link key={anggota.id} href={`/dosen-publik/${anggota.userId}`} className="flex items-center gap-3 p-2 -mx-2 rounded-lg hover:bg-zinc-50 transition-colors group">
                                                    ...
                                                    <p className="font-medium text-zinc-700 text-sm group-hover:text-blue-600 transition-colors">{anggota.user.name}</p>
                                                </Link>
                                            ))}
                                        </div>
                                    </div>
