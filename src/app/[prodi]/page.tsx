...
import { notFound } from "next/navigation"; // Import untuk menangani URL yang tidak terdaftar
...
// 1. Terima properti params untuk menangkap slug dari URL dinamis
export default async function HomePage({ params }: { params: Promise<{ prodi: string }> }) {
    // 1. Await params terlebih dahulu karena di Next.js terbaru params bertipe Promise
    const resolvedParams = await params;
    const slug = resolvedParams.prodi;

    // 2. Kueri Prisma sekarang akan menerima string murni, bukan Promise objek
    const prodiAktif = await prisma.prodi.findUnique({
        where: { slug }
    });

    // 3. Jika slug ngawur / tidak terdaftar, lempar ke halaman 404
    if (!prodiAktif) return notFound();

    // 4. Filter data statistik mahasiswa khusus prodi ini
    ...
    // 5. Filter widget jadwal aktifitas khusus prodi ini
    ...
    // 6. Pengunjung web tetap bersifat global/jurusan sesuai kode lama Anda
    const dataPengunjung = await prisma.pengunjung.findUnique({ where: { id: "global" } });
    const totalPengunjung = dataPengunjung?.total || 0;

    let newsTag = "programming"; // Default aman

        // Gunakan .includes() agar jenjang D3 dan D4 otomatis tercakup
        if (slug.includes("ti")) {
            newsTag = "webdev"; // Otomatis mencakup "d4-ti"
        } else if (slug.includes("komputer")) {
            newsTag = "iot"; // Otomatis mencakup "d3-tk", "d4-tk", dsb
        } else if (slug.includes("listrik")) {
            newsTag = "electronics"; // Otomatis mencakup "d3-te", "d4-te", dsb
        }

    let techNews = [];
    try {
        const res = await fetch(`https://dev.to/api/articles?tag=${newsTag}&per_page=8`, { 
        ...
        <div className="min-h-screen bg-white">
            <section className="relative overflow-hidden pt-28 pb-36">
                <div className="absolute inset-0">
                    <Image fill src={`/images/${prodiAktif.slug}.jpg`} alt="Kampus Politeknik Negeri Manado" className="object-cover" />
                ...
                <div className="relative container mx-auto px-4 text-center">
                    ...
                    {/* 7. Ubah nama Program Studi menjadi dinamis sesuai database */}
                    <h1 className="text-4xl md:text-6xl lg:text-7xl font-extrabold tracking-tight text-white mb-6 animate-fade-in-up delay-100" style={{ animationFillMode: 'both' }}>
                        ...
                        <span className="bg-linear-to-r from-blue-400 via-cyan-300 to-blue-400 bg-clip-text text-transparent animate-gradient">
                            {prodiAktif.nama.replace("Sarjana Terapan", "").replace("D4", "").trim()}
                        </span>
                    </h1>
                    <p className="text-lg md:text-xl text-blue-200/80 mb-10 max-w-2xl mx-auto animate-fade-in-up delay-200" style={{ animationFillMode: 'both' }}>
                        Menghasilkan lulusan unggul, inovatif, dan siap kerja di lingkup kompetensi {prodiAktif.nama} Politeknik Negeri Manado.
                    </p>
                </div>
                ...
                <div className="container mx-auto px-4 max-w-7xl">
                    <ScrollAnimate>
                        <div className="text-center max-w-2xl mx-auto mb-16">
                            ...
                            {/* 8. Dinamisasi sub-deskripsi prodi */}
                            <p className="text-zinc-500">Informasi akademik, perkembangan mahasiswa, dan jadwal perkuliahan {prodiAktif.nama}.</p>
                        </div>
                    </ScrollAnimate>
                                    ...
                                    {jadwalTerbaru ? (
                                        <div className="bg-white/80 backdrop-blur-sm rounded-xl p-4 border border-white shadow-sm">
                                            ...
                                            {jadwalTerbaru.gambarUrls && jadwalTerbaru.gambarUrls.length > 0 ? (
                                                ...
                                                // 9. Sesuaikan tautan navigasi internal dengan menyertakan slug prodi
                                                <Link href={`/${slug}/galeri/aktifitas/${jadwalTerbaru.id}`} className="w-full inline-flex items-center justify-center gap-2 bg-indigo-100 hover:bg-indigo-200 text-indigo-700 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors">
                                                    Lihat Detail
                                                </Link>
                                            )}
                                        </div>
                ...
                <div className="container mx-auto px-4">
                    <ScrollAnimate>
                        <div className="text-center max-w-2xl mx-auto mb-16">
                            ...
                            <p className="text-zinc-500">Kenali lebih dekat komponen utama yang menjadikan {prodiAktif.nama} unggul dan terdepan.</p>
                        </div>
                    </ScrollAnimate>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
                        <ScrollAnimate delay={0} className="h-full">
                            <div className="bg-white p-8 rounded-2xl shadow-sm border border-zinc-100 text-center card-hover group relative overflow-hidden h-full flex flex-col">
                                ...
                                <Link href={`/${slug}/dosen-publik`} className="inline-flex items-center text-blue-600 font-semibold hover:text-blue-700 transition-colors group/link">
                        ...
                        <ScrollAnimate delay={150} className="h-full">
                            <div className="bg-white p-8 rounded-2xl shadow-sm border border-zinc-100 text-center card-hover group relative overflow-hidden h-full flex flex-col">
                                ...
                                <Link href={`/${slug}/kurikulum-publik`} className="inline-flex items-center text-emerald-600 font-semibold hover:text-emerald-700 transition-colors group/link">
                        ...
                        <ScrollAnimate delay={300} className="h-full">
                            <div className="bg-white p-8 rounded-2xl shadow-sm border border-zinc-100 text-center card-hover group relative overflow-hidden h-full flex flex-col">
                                ...
                                <Link href={`/${slug}/galeri`} className="inline-flex items-center text-amber-600 font-semibold hover:text-amber-700 transition-colors group/link">
                    ...
                    <ScrollAnimate>
                        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 max-w-7xl mx-auto px-4 sm:px-12">
                            <div className="max-w-2xl">
                                ...
                                <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-zinc-900 mb-4">Update Dunia Teknologi</h2>
                            ...
                            <div className="mt-6 md:mt-0 hidden sm:block">
                                <Link href={`https://dev.to/t/${newsTag}`} target="_blank" className="text-sm font-semibold text-blue-600 hover:text-blue-800 flex items-center transition-colors">
