...
import { upsertStatistik, deleteStatistik, getStatistik } from "@/src/app/actions/statistik"; // 1. Import getStatistik
import { getActiveProdiId } from "@/src/app/actions/prodi-context"; // 2. Import pembaca cookie prodi
...
export default async function StatistikPage() {
    // 3. Gunakan Server Action yang sudah terfilter berdasarkan prodi aktif
    const statistik = await getStatistik();

    // 4. Ambil informasi nama prodi untuk membuat teks deskripsi dinamis
        ...
        <div className="space-y-6">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    {/* 5. Teks deskripsi dinamis */}
                    <p className="text-zinc-500">
                        Kelola data pendaftar dan lulusan {prodiAktif ? prodiAktif.nama : "Program Studi"} untuk grafik di Landing Page.
                    </p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-1">
                    <form action={upsertStatistik} className="space-y-4 bg-white p-5 rounded-md border sticky top-6">
                        ...
                        <p className="text-xs text-zinc-500 mb-4">Jika tahun sudah ada di prodi ini, data akan otomatis diperbarui.</p>
                            ...
                            {statistik.length === 0 && (
                                <TableRow>
                                    <TableCell colSpan={5} className="text-center text-zinc-500 h-32">
                                        Belum ada data statistik tahunan yang diinput untuk prodi ini.
                                    </TableCell>
                                </TableRow>
                            )}
                        </TableBody>
